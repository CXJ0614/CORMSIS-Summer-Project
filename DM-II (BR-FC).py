...
def forward_construction(uam_distance_matrix, demand_uam_distance_matrix, capacities, costs, L, U, T, V):
    ...
    while remaining_capacity > 0 or not np.all(np.min(demand_uam_distance_matrix[:, solution] / V, axis=1) <= T) or total_cost > U:
        ...
        if not candidate_list:  # If there are no candidates left, return an empty solution, the reason why can't find a feasible result is that the evluation function parameter setting is not suitable
            return []
        ...
        def evaluate_candidate(node):
            ...
            return alpha * min_distance + beta * capacities[node] + gamma * (U - costs[node]) + lambda_ * covered_demands
...
if __name__ == "__main__":
    ...
    tabu_solution, tabu_objective = tabu_search(uam_distance_matrix, demand_uam_distance_matrix, capacities, costs, L, U, T, V, P, initial_solution, max_iteration=10, tabu_tenure=5)
