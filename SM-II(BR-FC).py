...
def forward_construction(distance_matrix, demand_uam_distances, capacities, L, T, V):
    solution = []
    ...
    while remaining_capacity > 0 or not np.all(
            np.min(demand_uam_distances[:, solution] / V, axis=1) <= T):
        ...
        def evaluate_candidate(node):
            min_distance = min([distance_matrix[node, j] for j in solution])
            r = V * T
            covered_patients = np.sum(demand_uam_distances[:, node] <= r)
