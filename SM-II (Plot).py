...
T = 60  # Maximum acceptable travel time (Min)
...
alpha_weight = 0.4  # Distance weight
beta_weight = 0.4  # Capacity weight
lambda_weight = 0.2  # Time weight
...
# BR-FC Algorithm to generate initial solution with the new evaluation function
def forward_construction(hospital_distance_matrix, patient_hospital_distance_matrix, capacities, L, T, V):
    solution = []

    # Step 1: Create a sorted list of edges based on distance (in descending order)
    edges = [(i, j, hospital_distance_matrix[i, j]) for i in range(n) for j in range(i + 1, n)]
    ...
    while remaining_capacity > 0 or not np.all(
            np.min(patient_hospital_distance_matrix[:, solution] / V, axis=1) <= T):
        ...
        def evaluate_candidate(node):
            min_distance = min([hospital_distance_matrix[node, j] for j in solution])
            r = V * T
            covered_patients = np.sum(patient_hospital_distance_matrix[:, node] <= r)
            return alpha_weight * min_distance + beta_weight * capacities[node][0] + lambda_weight * covered_patients
...
def tabu_search(distance_matrix, demand_uam_distances, capacities, delays, L, T, alpha, beta, initial_solution, max_iteration=20, tabu_tenure=5):
    ...
    df_history.to_csv('tabu_search_SM2_history.csv', index=False)
...
def visualize_locations(uam_locations, demand_locations, selected_uam, title):
    ...
    if selected_uam:
        ...
        for j in range(len(demand_locations)):
            demand = demand_locations[j]
            closest_uam = min(selected_uam, key=lambda i: np.linalg.norm(demand - uam_locations[i]))
            plt.plot([demand[0], uam_locations[closest_uam, 0]],
                     [demand[1], uam_locations[closest_uam, 1]], 'gray', linestyle='--')
...
if __name__ == "__main__":
    ...
    initial_solution = forward_construction(distance_matrix, demand_uam_distances, capacities, L, T, V)
  ...
  # End time for performance measurement
    ...
    # Visualize results
    visualize_locations(uam_locations, demand_locations, initial_solution, "Initial Solution with BR-FC Algorithm")
    visualize_locations(uam_locations, demand_locations, best_solution, "The Location of UAM Stations of Stochastic Model")